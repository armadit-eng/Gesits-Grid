...
const BLEAdapter = (() => {
  ...
  class CapacitorBLE {
    ...
    async write(serviceUUID, charUUID, text) {
      ...
      // Encode text ke base64 dengan cara yang aman
      let b64;
      try {
      ...
      console.log('[BLE] write:', text.replace(/\r\n/g,'\\r\\n'), '→ b64 len:', b64.length);

      // Coba write, fallback ke writeWithoutResponse
      try {
        ...
        console.warn('[BLE] write failed, trying withoutResponse:', e.message);
        try {
          ...
          console.warn('[BLE] writeWithoutResponse also failed:', e2.message);
          // Non-fatal — jangan crash app
        }
      }
    }
